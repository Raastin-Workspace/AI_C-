...
const std::string Strategy::INDIRECT_FREEKICK_OUR_FORMATION_NEX_CONF = "indirect-freekick-our-formationNex.conf";
...
Strategy::read( const std::string & formation_dir )
{
    ...
    M_indirect_freekick_our_formation_nex = readFormation( configpath + INDIRECT_FREEKICK_OUR_FORMATION_NEX_CONF );
    if ( ! M_indirect_freekick_our_formation_nex )
    {
        std::cerr << "Failed to read indirect freekick our formation" << std::endl;
        return false;
    }
...
//            return M_kickin_our_formation;
            if(wm.opponentTeamName() != "Nexus")
                return M_indirect_freekick_our_formation;
            else
                return M_indirect_freekick_our_formation_nex;
        }
        else
        {
    ...
    {
//        return M_indirect_freekick_our_formation;
        if ( wm.ball().pos().x > ServerParam::i().theirPenaltyAreaLineX()-10.0
             /*&& wm.ball().pos().absY() < ServerParam::i().penaltyAreaHalfWidth() + 5.0*/ )
        {
//            return M_indirect_freekick_our_formation;
            if(wm.opponentTeamName() != "Nexus")
                return M_indirect_freekick_our_formation;
...
//                return M_indirect_freekick_our_formation;
         ...
         || wm.gameMode().type() == GameMode::FoulPush_ )
    {
        if ( wm.gameMode().side() == wm.ourSide() )
        {
            ...
            // opponent (indirect) free kick
            //
            if ( wm.ball().pos().x < ServerParam::i().ourPenaltyAreaLineX() + 7.0
...
Strategy::get_normal_dash_power( const WorldModel & wm )
{
    ...
    else if ( wm.self().stamina() < ServerParam::i().staminaMax() * 0.5 && wm.self().unum() < 9 )
    {
        s_recover_mode = true;
    }
    else if ( wm.self().stamina() < ServerParam::i().staminaMax() * 0.38 && wm.self().unum() >= 9 )
    {
        s_recover_mode = true;
    }
...
//        if(wm.self().unum()>8 && wm.existKickableOpponent())
//            111;
        dlog.addText( Logger::TEAM,
...
//    }
    ...
    if(wm.ball().pos().x>34.0 /*&& wm.ball().pos().absY()<23.0*/ && wm.self().unum()>6)
        dash_power*=1.2;

    if(wm.ball().pos().x>34.0 && wm.self().unum()>8)
        dash_power*=1.2;

    if(wm.ball().pos().x>25.0 && wm.self().unum()>8)
        dash_power*=1.2;

    if(wm.ball().pos().x<20.0 && wm.self().unum()<6){
        //dash_power=doubleNormalize(dash_power,40.0,100);
        if(dash_power<40.0)
            dash_power=40.0;
        if(dash_power>100.0)
            dash_power=100.0;
    ...
    if(wm.ball().pos().x<10.0 && wm.self().unum()==6){
        //dash_power=doubleNormalize(dash_power,40.0,100);
        if(dash_power<40.0)
            dash_power=40.0;
        if(dash_power>100.0)
    ...
    if(wm.self().unum()>8 && wm.ball().pos().x>20  /*&& wm.ball().pos().x<45*/ ){
...
//            dash_power*=1.3;
    ...
    if(wm.self().unum()<4 && wm.ball().pos().x < 0  /*&& wm.ball().pos().x<45*/ ){
...
//            dash_power*=1.3;
    ...
    if(wm.self().unum()>=4 && wm.self().unum()<6 && wm.ball().pos().x < 0  /*&& wm.ball().pos().x<45*/ ){

        if(wm.ball().pos().x<wm.self().pos().x && wm.ball().pos().dist(wm.self().pos())< 35 )
            dash_power*=1.2;
//        else
//            dash_power*=1.2;
//        if( wm.ball().pos().x< wm.offsideLineX()-2 && wm.offsideLineCount()<6)
...
//        dash_power=100.0;
    ...
    if ( wm.self().stamina() < 3000 && wm.self().unum()>8)
    {
        dash_power= 0 ;
    }
    else if ( wm.self().stamina() < 2500 &&  wm.self().unum()>6)
    {
        dash_power= 0 ;
    }
    else if ( wm.self().stamina() < 1000  )
    {
        dash_power= 0 ;
    }
...
SituationType Strategy::getSitu(const rcsc::WorldModel & wm ){
    ...
    if ( opp_min <= our_min - 2 )
    {
        return Defense_Situation;

    }

    if ( our_min <= opp_min - 2 )
    {
        return  Offense_Situation;
