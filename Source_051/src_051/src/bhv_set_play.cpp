...
//            return Bhv_SetPlayIndirectFreeKick().execute( agent );
    ...
    if ( wm.gameMode().isOurSetPlay( wm.ourSide() ) )
    {
        ...
        if(wm.ball().pos().x > 30.0 && wm.ball().pos().absY() < 18.0)
            return Bhv_SetPlayIndirectFreeKick().execute( agent );
        else
...
Bhv_SetPlay::is_delaying_tactics_situation( const PlayerAgent * agent )
{
    ...
    if(our_score <= opp_score && wm.ball().pos().x > 30  ){
        if(wm.time().cycle() > 2800 && wm.time().cycle() < 2999  )
            return false;
        else if(wm.time().cycle() > 5800 && wm.time().cycle() < 5999 )
            return false;
    }
