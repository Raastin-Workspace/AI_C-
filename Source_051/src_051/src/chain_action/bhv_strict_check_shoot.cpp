...
Bhv_StrictCheckShoot::execute(PlayerAgent * agent )
{
    ...
    bool can = true;// getCount(agent, best_shoot->target_point_,2.5)<3 ;
    if ( one_step_speed > best_shoot->first_ball_speed_ * 0.99  && can)
    {
...
//             agent->setNeckAction( new Neck_TurnToGoalieOrScan( -1 ) );
           // agent->setNeckAction( new Neck_TurnToPoint( best_shoot->target_point_ ) );
    ...
    if ( can && Body_SmartKick( best_shoot->target_point_,
                         best_shoot->first_ball_speed_,
...
int Bhv_StrictCheckShoot::getCount(rcsc::PlayerAgent * agent,rcsc::Vector2D pos,double PM){
