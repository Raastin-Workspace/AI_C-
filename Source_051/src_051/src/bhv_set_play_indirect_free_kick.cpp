...
Bhv_SetPlayIndirectFreeKick::doOffenseMove( PlayerAgent * agent )
{
                           ...
                           ).execute( agent ) )
    {
        ...
        Body_TurnToPoint(goal_pos).execute(agent);
...
//    if(wm.self().pos().x < -51.0 ){



//        Line2D goalLine(Vector2D(-52.0,0.0 ),90.0);
//        Line2D Line(wm.ball().pos(),target_point);
//        if(wm.ball().pos().dist(target_point) < 9.5 )
//            adjusted_point = goalLine.intersection(Line);
