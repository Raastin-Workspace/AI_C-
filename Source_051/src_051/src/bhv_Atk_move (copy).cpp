...
//    agent->setNeckAction( new Neck_OffensiveInterceptNeck() );
    ...
    //-----------------------------------------------
    // tackle
    if ( Bhv_BasicTackle( 0.8, 80.0 ).execute( agent ) && !(wm.ball().pos().x>42 && fabs(wm.ball().pos().y)>10))
    {
        return true;
    }

    if( (b->tackle(agent) || b->bTackle(agent)) && Bhv_BasicTackle(0.8 , 80.0).execute2(agent) && wm.ball().pos().x<36){
...
//            target_point.x=wm.offsideLineX()-2;
    ...
    if(wm.ball().pos().x<42 && wm.ball().pos().x>-10 &&  a->goTo2(agent,target_point,0,dash_power,dist_thr)){
        ...
        return a->goTo2(agent,target_point,0,dash_power,dist_thr);
    }
