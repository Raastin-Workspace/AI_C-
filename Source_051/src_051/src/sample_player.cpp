...
////    if(!wm.self().goalie() && kickable && Kicking.cycle+Kicking.step>wm.time().cycle()){

////        Body_SmartKick  smart(Kicking.end,Kicking.speed,Kicking.M_speed,Kicking.step-1);

////        if(smart.execute(this)){

////            b->setKicking(wm,smart);
////            this->setNeckAction( new Neck_TurnToPoint(Kicking.end));
//////            this->addSayMessage(new PassMessage(nr,Kicking.end,m->startKick(wm,smart),vel));
////            return true;
...
////    if(!wm.self().goalie() && kickable &&  m->SHOOOOOOT(this,Kicking)){
...
SamplePlayer::doPreprocess()
{
    ...
    bool kickable = this->world().self().isKickable();
    const WorldModel & wm = this->world();
    // cout<<wm.time()<<"  "<<__FILE__<<" startpreproces "<<__LINE__<<" "<<wm.self().unum()<<endl;
...
//        this->setViewAction( new View_Tactical() );
//        this->setNeckAction( new Neck_TurnToBallOrScan() );
        cout<<wm.time().cycle()<<"  frozen "<<endl;
        this->setNeckAction( new Neck_TurnToPoint(m->where(this)));
         ...
         || wm.gameMode().type() == GameMode::AfterGoal_ )
    {
        ...
        Bhv_CustomBeforeKickOff( move_point ).execute( this );
        this->setViewAction( new View_Tactical() );
        cout<<wm.time().cycle()<<"  beforekickOff "<<endl;
        return true;
    }
    ...
    if ( ! wm.self().posValid() )
    {
        dlog.addText( Logger::TEAM,
                      __FILE__": invalid my pos" );
        Bhv_Emergency().execute( this ); // includes change view
        cout<<wm.time().cycle()<<"  emergency "<<endl;
        return true;
    }
...
//     ball localization error
              ...
              && wm.ball().seenPosCount() > count_thr + 10 ) )
    {
        ...
        this->setViewAction( new View_Tactical() );
        Bhv_NeckBodyToBall().execute( this );
        cout<<wm.time().cycle()<<"  searchBall "<<endl;
        return true;
    }
...
//        cout<<wm.time().cycle()<<"  tryNnextStep : "<<endl;
        cout<<wm.time().cycle()<<"  nextStep "<<endl;

        if(wm.ourPlayer(Kicking.reciverUnum) && wm.ourPlayer(Kicking.reciverUnum)->posCount() <= 1 ){

            Kicking.end = wm.ourPlayer(Kicking.reciverUnum)->pos() + Kicking.reciverVector;
        }
        else{
        ...
        if( smart.execute(this) &&
            ( m->OCI(this,toVecPos(Shoot().startKick(wm,smart)),toVecPos(Kicking.end),smart.sequence().score_,10,0.3,Shoot().stepKick(smart),0,false,0) ||
              Shoot().OCI(this,(Shoot().startKick(wm,smart)),(Kicking.end),smart.sequence().score_,10,0.3,Shoot().stepKick(smart),0,false,0)||
              Kicking.reciverUnum ==666) ){

//            cout<<wm.time().cycle()<<"  nextStep : accept"<<endl;

            Vector2D vel=Vector2D::polar2vector(smart.sequence().speed_,(Kicking.end-m->startKick(wm,smart)).dir().degree());
            this->addSayMessage(new PassMessage(Kicking.reciver,Kicking.end,m->startKick(wm,smart),vel));
            b->setKicking(wm,smart);
            this->setNeckAction( new Neck_TurnToPoint(Kicking.end));
    ...
    else if(!wm.self().goalie() && ! wm.existKickableTeammate() && (Kicking.reciver > 6 && Kicking.end.x>-20) && Kicking.cycle+Kicking.step + 5 > wm.time().cycle()){

//        cout<<wm.time().cycle()<<"  tryNnextStep : "<<endl;



        if( wm.ball().pos().dist(Kicking.end) < 5.0 )
            ;
        else{


            Kicking.speed*=BALL_DECAY;

            Vector2D vel=Vector2D::polar2vector(Kicking.speed,(Kicking.end-wm.ball().pos()).dir().degree());
            ...
            this->addSayMessage(new PassMessage(Kicking.reciver,Kicking.end,wm.ball().pos(),vel));
            this->setNeckAction( new Neck_TurnToPoint(Kicking.end));
    ...
    // check shoot chance
    //
//    if(!wm.self().goalie() && kickable &&  Shoot().execute(this)){
...
//    }
    if(!wm.self().goalie() && kickable &&  Shoot().execute23(this,Kicking)){
        cout<<wm.time().cycle()<<"  shoot23: "<<endl;
    ...
    if(!wm.self().goalie() && kickable &&  Shoot().SHOOOOOOT(this,Kicking)){

        Body_SmartKick  smart(Kicking.end,Kicking.speed,Kicking.M_speed,Kicking.step);

        if(smart.execute(this) && ( m->OCI(this,toVecPos(Shoot().startKick(wm,smart)),toVecPos(Kicking.end),smart.sequence().score_,10,0.3,Shoot().stepKick(smart),0,false,0) || Shoot().OCI(this,(Shoot().startKick(wm,smart)),(Kicking.end),smart.sequence().score_,10,0.3,Shoot().stepKick(smart),0,false,0))){
            this->setNeckAction( new Neck_TurnToPoint(Kicking.end));
    ...
    if ( doShoot() )
    {
                    cout<<wm.time().cycle()<<"  doshoot"<<endl;
        return true;
    }
//    if(!wm.self().goalie() && kickable &&  Shoot().execute(this)){
//        cout<<wm.time().cycle()<<"  shoot1: "<<endl;
...
//        }
//    }
////    if(ShootIO().execute(this)){


//        cout<<this->world().time().cycle()<<" did IO SHooT "<<endl;
//        this->setNeckAction( new Neck_TurnToPoint(m->where(this)));
//        return true;

//    }
//    if(!wm.self().goalie() && wm.ball().pos().x < 45 && kickable &&  m->SHOOOOOOT(this,Kicking) ){

//        Body_SmartKick  smart(Kicking.end,Kicking.speed,Kicking.M_speed,Kicking.step);

//        if(smart.execute(this)){
//            this->setNeckAction( new Neck_TurnToPoint(Kicking.end));
//            cout<<wm.time().cycle()<<"  shoot3 : "<<endl;
...
//    }
    ...
    //
    int nr;
    if ( !wm.self().goalie() && kickable && m->execute ( this,Kicking ) )
    {
        ...
        if(smart.execute(this)){
            ...
            this->setNeckAction( new Neck_TurnToPoint(Kicking.end));
            this->addSayMessage(new PassMessage(Kicking.reciver,Kicking.end,m->startKick(wm,smart),vel));
            cout<<wm.time().cycle()<<"  shayan : "<<Kicking.reciver<<"  "<<Kicking.end<<endl;
...
//    else
    ...
    if(wm.ball().pos().x < 30 && !wm.self().goalie() && kickable && m->throughPassSabok(this,Kicking,nr)){
    ...
    if(wm.ball().pos().x < 30 && !wm.self().goalie() && kickable && m->leadingPassSabok(this,Kicking,nr)){
    ...
    if(wm.ball().pos().x < 30 && !wm.self().goalie() && kickable && m->directPassSabok(this,Kicking,nr)){
    ...
    if ( this->doIntention() )
    {
        cout<<wm.time().cycle()<<"   intention "<<wm.self().unum()<<endl;

//        if(wm.ball().pos().dist(wm.self().pos())<3){
//            this->setNeckAction( new Neck_TurnToPoint(m->where(this)));
//        }
        this->setNeckAction( new Neck_TurnToPoint(m->where(this,3)));
    ...
    if ( doHeardPassReceive() )
    {
        //cout<<wm.time().cycle()<<"  heardPass: "<<endl;
        cout<<wm.time().cycle()<<"  reciving "<<endl;
        return true;
    }

    // cout<<wm.time()<<"  "<<__FILE__<<" endpreproces "<<__LINE__<<" "<<wm.self().unum()<<endl;
    return false;
}
...
SamplePlayer::doShoot()
{
    ...
    if(wm.ball().pos().x<45  || (wm.ball().pos().x>50 && fabs(wm.ball().pos().y)>9.0) )
        return false;
    if(wm.ball().pos().x>48 && fabs(wm.ball().pos().y)>9.0 )
        return false;
...
//    if(wm.ball().pos().x > -20 && wm.ball().pos().x < 36)
//        return false;
//    if(wm.ball().pos().absY() > 18 )
//        return false;
...
SamplePlayer::doHeardPassReceive()
{
    ...
    if ( wm.audioMemory().passTime() != wm.time()
         || wm.audioMemory().pass().empty()
         || wm.audioMemory().pass().front().receiver_ != wm.self().unum() || wm.self().isKickable())
    {
    ...
    int closest = wm.teammatesFromBall().front()->unum();

    bool recived = false;

    if(wm.existKickableTeammate() && sender!= closest )
        recived =true;
    ...
    if ( ! /*wm.existKickableTeammate()*/recived
         && wm.ball().posCount() <= 1
         && wm.ball().velCount() <= 1
//         && self_min < 20
         //&& intercept_pos.dist( heard_pos ) < 3.0 ) //5.0 )
         )
    {
        ...
        if(goalChance && wm.interceptTable()->selfReachCycle() <2 )
            this->setNeckAction( new Neck_TurnToPoint(m->where(this,2)) );
        else
            this->setNeckAction( new Neck_TurnToBall() );
    }
    else if( wm.ball().posCount() >4
            && wm.ball().velCount() >4 &&
             fabs(AngleDeg::normalize_angle(((wm.ball().pos()-wm.self().pos()).dir().degree())-wm.self().body().degree()))>107){
