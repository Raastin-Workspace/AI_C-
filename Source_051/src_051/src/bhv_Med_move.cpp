 ...
 */
bool
Bhv_MedMove::execute( PlayerAgent * agent )
{
...
//    agent->setNeckAction( new Neck_OffensiveInterceptNeck() );
    ...
    if( (b->tackle(agent) || b->bTackle(agent) ) && wm.ball().pos().x<36 && Bhv_BasicTackle(0.8 , 80.0).execute2(agent)){
...
//        agent->setAction( new Neck_OffensiveInterceptNeck() );
...
//        cout<<"cycle  :  "<<wm.time().cycle()<<"  blockPos:   "<<posBlock<<endl;
    ...
    Positioning * a;
    if(/*a->offenseMode(agent) && */ wm.self().unum()>6 && wm.ball().pos().x>-20.0 && Strategy::getSitu(wm) == Offense_Situation)
    ...
    dlog.addText( Logger::TEAM,
                  __FILE__": Bhv_MedMove target=(%.1f %.1f) dist_thr=%.2f",
                  target_point.x, target_point.y,
                  dist_thr );

    agent->debugClient().addMessage( "MedMove%.0f", dash_power );
...
bool    Bhv_MedMove::turnNeck(PlayerAgent *agent){
