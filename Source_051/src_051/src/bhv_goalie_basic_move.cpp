...
Bhv_GoalieBasicMove::execute( PlayerAgent * agent )
{
    ...
    if(doAddDeepCross(agent)){

        // std::cout<<wm.time().cycle()<<" Goalie : "<<__FILE__<<"  "<<__LINE__<<std::endl;
        return true;
    }
    //////////////////////////////////////////////////////////////////
    // tackle
                                 ...
                                 const Vector2D & move_point )
{
    ...
    if ( ( ! wm.existKickableOpponent() && opp_min_cyc >= 4 )
         || wm.ball().distFromSelf() > 18.0 || std::fabs( move_point.x - wm.self().pos().x ) > x_buf )
    {
        double dash_power = getBasicDashPower( agent, move_point );
...
bool    Bhv_GoalieBasicMove::doAddDeepCross(rcsc::PlayerAgent * agent ){

    const WorldModel & wm = agent->world();

    int     selfReachCycle = wm.interceptTable()->selfReachCycle(),
            oppReachCycle = wm.interceptTable()->opponentReachCycle(),
            ttReachCycle = wm.interceptTable()->teammateReachCycle();



    if( !(ttReachCycle + 5 <= oppReachCycle ) && selfReachCycle  < ttReachCycle && selfReachCycle + 1  < oppReachCycle ){
        agent->setNeckAction( new Neck_TurnToBall() );
    ...
    static bool doThat = false;

    if(wm.ball().pos().x > -10 )
        doThat = true;

    else if( wm.ball().pos().x < -20.0 || wm.gameMode().type() != GameMode::PlayOn )
        doThat = false;
    ...
    double point_x = -32.0;
//    if( wm.ball().pos().x > -30.0 && wm.ball().pos().x < -20.0)
//        point_x = -45.0;

    double point_y = 0.0;
    Vector2D stand = Vector2D(point_x , point_y);
    Line2D X_line = Line2D( stand , 90.0 );
    Line2D lineToGoal = Line2D(wm.ball().pos(), Vector2D(-52.5 , 0.0 ));

    stand  = X_line.intersection(lineToGoal);

    double dash_power = getBasicDashPower( agent, stand );

    static bool khatari = false;
    if(wm.self().pos().dist(stand) > 4.0 )
        khatari = true;
    else if (wm.self().pos().dist(stand) < 2 )
        khatari = false;
    ...
    if ( ! wm.existKickableOpponent()
         && wm.ball().distFromSelf() > 20.0 )
    {
        if ( ! Body_GoToPoint( stand, 0.5, dash_power
                               ).execute( agent ) )
        {
            AngleDeg body_angle = ( wm.self().body().degree() > 0.0
                                    ? 90.0
